...
void get_driver()
{
	...
	{
		m_HWDeviceTypes.push_back(type);
...
int initHWDecoder(AVCodecContext* avcodec_context, const AVCodec* codec, AVBufferRef* hw_device_ctx)
{
	get_driver();
	if (!codec)
		return -1;
		...
		if (config->methods & AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX)       // 判断是否是设备类型
		{
			for (auto i : m_HWDeviceTypes)
			{
				if (config->device_type == AVHWDeviceType(i))                 // 判断设备类型是否是支持的硬件解码器
				{
					...
					// 打开指定类型的设备，并为其创建 AVHWDeviceContext。
					...
					memset(m_codes, 0, 128);
					sprintf_s(m_codes, "%s", av_hwdevice_get_type_name(config->device_type));
					printf("打开硬件解码器:%s\n", m_codes);

					avcodec_context->hw_device_ctx = av_buffer_ref(hw_device_ctx);  // 创建一个对AVBuffer的新引用。
					//avcodec_context->get_format = get_hw_format;                    // 由一些解码器调用，以选择将用于输出帧的像素格式
					return 0;
				}
			}
		}
	...
	AVCodecContext **input_codec_context, AVCodecContext **video_codec_context)
{
	...
	{
		if ((*input_format_context)->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)//打开视频解码器
		{
			...
			*video_codec_context = avcodec_alloc_context3(video_codec);			
			...
			initHWDecoder(*video_codec_context, video_codec, hw_device_ctx);
...
AVCodecContext *enh265 =NULL;
//缓冲区分配内存(只有指定了AVFrame的像素格式、画面大小才能真正分配内存)
unsigned char *out_buffer;
AVFrame* m_frameTemp = av_frame_alloc();
	...
	AVCodecContext **video_codec_context)
{
	...
	*audio_codec_context = avcodec_alloc_context3(audio_codec);
	...
	//video_codec = avcodec_find_encoder_by_name("hevc_nvenc");
	//if (video_codec == NULL)
	//{
	...
	video_codec2 = avcodec_find_decoder(AV_CODEC_ID_HEVC);
	if (video_codec2 == NULL)
	{
		printf("video_codec2 Codec not found !\n");
		return -1;
	}
	...
	*video_codec_context = avcodec_alloc_context3(video_codec);
	...
	initHWDecoder(*video_codec_context, video_codec2, hw_device_ctx2);
	enh265 = avcodec_alloc_context3(video_codec2);
	if (enh265 == NULL)
	{
		printf("Could not allocate audio codec context!\n");
		return -1;
	...
	{
		if ((*intput_format_context)->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
		{
	...
	avcodec_copy_context(enh265,*video_codec_context);
	enh265->codec = video_codec2;
	...
	if (avcodec_open2(enh265, video_codec2, NULL) < 0)
	{
		return -1;
	}
	return 0;
...
AVFrame* pFramein_gpu = av_frame_alloc();
static int video_decodec_encodec_write(AVFormatContext **output_format_context, AVFormatContext **input_format_context, AVCodecContext **output_codec_context, AVCodecContext **input_codec_context, AVPacket **pkt, cv::Mat &yuv420_mat, cv::Mat &bgr_mat)
{
	int ret;
	...
	AVFrame *video_frame_h265 = av_frame_alloc();
	AVPacket  *video_pkt = av_packet_alloc();
	struct SwsContext *img_convert_ctx; //图像转换数据结构
	struct SwsContext *img_convert_ctx_2; //图像转换数据结构
	...
	while (true)
	{
		...
		else
		{
			...
			av_hwframe_transfer_data(pFramein_gpu, video_frame, 0);//从GUP的显存复制NV12出来

			img_convert_ctx = sws_getContext(
				video_frame->width,                   // 输入图像的宽度
				video_frame->height,                  // 输入图像的高度
				(AVPixelFormat)pFramein_gpu->format,   // 输入图像的像素格式
				video_frame->width,                                 // 输出图像的宽度
				video_frame->height,                                 // 输出图像的高度
			...
			sws_scale(img_convert_ctx,
				pFramein_gpu->data,
				pFramein_gpu->linesize,
				0,
				video_frame->height,
				m_frameTemp->data,
				m_frameTemp->linesize);

			m_frameTemp->width = video_frame->width;
			m_frameTemp->height = video_frame->height;
			m_frameTemp->format =0;

			ret = avcodec_send_frame(*output_codec_context, m_frameTemp);
			...
			while (true)
			{
				...
				else
				{
					...
					ret = avcodec_send_packet(enh265, video_pkt);
					if (ret != 0)
					{
						printf("avcodec_send_frame is error to video");
						return -1;
					...
					while (true)
					{
						ret = avcodec_receive_frame(enh265, video_frame_h265);
						if (ret != 0)
						{
							break;
						}

						av_hwframe_transfer_data(pFramein_gpu, video_frame_h265, 0);//从GUP的显存复制NV12出来
						img_convert_ctx_2 = sws_getContext(
							video_frame_h265->width,                   // 输入图像的宽度
							video_frame_h265->height,                  // 输入图像的高度
							(AVPixelFormat)pFramein_gpu->format,   // 输入图像的像素格式
							video_frame_h265->width,                                 // 输出图像的宽度
							video_frame_h265->height,                                 // 输出图像的高度
							AV_PIX_FMT_YUV420P,                           // 输出图像的像素格式
							SWS_BILINEAR,                         // 选择缩放算法
							NULL, NULL, NULL);

						sws_scale(img_convert_ctx_2,
							pFramein_gpu->data,
							pFramein_gpu->linesize,
							0,
							video_frame->height,
						...
						memcpy(yuv420_mat.data, m_frameTemp->data[0], enh265->width * enh265->height);
						memcpy(yuv420_mat.data + enh265->width * enh265->height, m_frameTemp->data[1], enh265->width * enh265->height / 4);
						memcpy(yuv420_mat.data + enh265->width * enh265->height * 5 / 4, m_frameTemp->data[2], enh265->width * enh265->height / 4);

						// 转换颜色空间
						cv::cvtColor(yuv420_mat, bgr_mat, cv::COLOR_YUV2BGR_I420);
						sws_freeContext(img_convert_ctx_2);
					}
...
int TS_Format_conversion_MP4(const char *inputfile, const char *outputfile,int W ,int H,int fps)
{
	...
	video_index = streams_init(ifmt_ctx, ofmt_ctx, &video_output_codectx, &audio_out_codecctx);//返回视频的index
	if (video_index < 0)
	{
		printf("streams_init is error");
		return -1;
	}
	//if (ifmt_ctx->streams[!video_index]->codecpar->sample_rate != 0)//判断是否需要初始化重采样
	//{
	...
	//	}
	//}
	//if (ifmt_ctx->streams[!video_index]->codecpar->sample_rate != 0)//判断是否需要初始化缓冲区
	//{
	...
	cv::namedWindow("YUV", cv::WINDOW_NORMAL);
	cv::resizeWindow("YUV", W, H);
	cv::Mat yuv420_mat(enh265->height * 3 / 2, enh265->width, CV_8UC1);
	cv::Mat bgr_mat(enh265->height, enh265->height, CV_8UC3);
	out_buffer = (unsigned char *)av_malloc(av_image_get_buffer_size(AV_PIX_FMT_YUV420P, video_input_codectx->width, video_input_codectx->height, 1));
	av_image_fill_arrays(m_frameTemp->data, m_frameTemp->linesize, out_buffer,
		AV_PIX_FMT_YUV420P, video_input_codectx->width, video_input_codectx->height, 1);
	while (0 <= av_read_frame(ifmt_ctx, pkt)) {
		...
		if (pkt->stream_index == video_index)
		{
			if (ifmt_ctx->streams[pkt->stream_index]->codecpar->codec_id == AV_CODEC_ID_HEVC)//判断是否为H264，决定是否复用和转码
			{				
			...
			{			
				video_decodec_encodec_write(&ofmt_ctx,&ifmt_ctx,&video_output_codectx,&video_input_codectx, &pkt, yuv420_mat, bgr_mat);
				cv::imshow("YUV", bgr_mat);
				cv::waitKey(1000/fps);
				av_free_packet(pkt);
				continue;
			}
